...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  # Enables policy and workload identity integrations.
  azure_policy_enabled      = true
  oidc_issuer_enabled       = true
  workload_identity_enabled = true

  # Uses a system node pool with optional autoscaling and subnet placement.
  default_node_pool {
    name                = "system"
    ...
    min_count = (
      var.enable_cluster_autoscaler ? var.node_min_count : null
    )
    max_count = (
      var.enable_cluster_autoscaler ? var.node_max_count : null
    )
  ...
  # Uses managed identity to avoid static credentials.
  identity {
    type = "SystemAssigned"
  }
